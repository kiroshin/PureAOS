...
//inline fun <reified T: Any> UiState<T>.onFailure(action: (error: Throwable?) -> Unit) {
...
inline fun <reified T: Any> UiState<T>.onReadyOrLoading(action: () -> Unit) {
    if (this is UiState.Ready || this is UiState.Loading) {
        action()
    }
}